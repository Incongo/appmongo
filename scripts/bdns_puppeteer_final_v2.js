  ...
  try {
    ...
    try {
      ...
      await page.waitForFunction(() => {
        ...
        const hayResultados = document.querySelectorAll('.convocatoria, .resultado, tr').length > 5;
        const hayTexto = document.body.innerText.includes('Título de la convocatoria');
      ...
      tablas.forEach((tabla) => {
        ...
        filas.forEach((fila, index) => {
          ...
          if (celdas.length >= 3) {
            ...
            const idMatch = titulo.match(/BDNS\|?(\d+)/) || 
                           enlace?.href?.match(/id=(\d+)/);
    ...
    if (convocatorias.length > 0) {
      ...
      // Guardar resultados
      fs.writeFileSync('convocatorias_bdns.json', JSON.stringify(convocatorias, null, 2));
